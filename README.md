...
## Рассылка дайджеста
Для еженедельной и ежедневной рассылок сгенерены соответствующие Mailer, а для них Job. Чтобы выполнять рассылку в заданное время установлены гемы resque и resque-scheduler. В файле config/rescue_scheduler.yml прописаны задачи. По идее job должны запускаться по расписанию, но, к сожалению, сходу завести не удалось. При запуске resque пишет, что задача запущена, но логирование показывает, что не вызывается метод perform у Job и при этом не выдает никакой ошибки. В логе видно, что задача попала в очередь и запустилась, но действия в функции perform не выполняются. При вызове метода из кода, напримеро, TestJob.perform_now все отрабатывает. Я пока не стал дальше разбираться.
